...
7. **Both sides**: strip whole-line `<page url ...>...</page>` tags AND whole-line nsync child-link lines (regex below). The `<page url>` strip catches what Notion serializes; the child-link strip catches what nsync renders locally. Together they ensure child adds/removes/renames/reorders never move either hash.
...
Lines matching the regex are nsync-managed; see `path-mapping.md` → "Child-link lines" for format, position semantics, and the auto-managed rule. `<page url>` is the input form Notion gives via `notion-fetch`; child-link is the local rendered form written by `/nsync:init` and regenerated by `/nsync:pull`.
