...
│   └── sub-agent-schemas.md    # JSON Schemas for the three sub-agent return shapes
...
10. **Reads parallelize, writes serialize.** Per-page `notion-fetch` loops (pull/status/diff/init bodies, commit staleness) emit batches of `NSYNC_READ_BATCH` concurrent calls in a single assistant message; `notion-create-pages` batches siblings per shared parent (`pages[]`, max 100); `notion-update-page` / `notion-move-pages` stay one call per page (Claude Code serializes writes, and Notion's ~3 req/s ceiling 429s write bursts). The `NSYNC_READ_BATCH` constant lives ONLY in `references/notion-mcp-cheatsheet.md` → "Concurrency, batching & rate limits"; commands reference it, never restate the number. Don't add a per-repo concurrency field to the manifest — it's intentionally hardcoded. Long loops must also surface progress **at least once per minute** (never silent >~60s) per that reference's "Progress reporting" subsection — commands point at it; the cadence rule is not restated in command bodies.
11. **Deterministic work runs in `scripts/nsync.py`, never in-context.** Every hash, normalization, child-link strip, snapshot→local diff, and `<page url>`-URL → UUID extraction goes through the helper (`hash` / `hash-batch` / `normalize` / `strip-childlinks` / `diff` / `extract-uuids` / `self-test`). The model must never compute a SHA-256, strip tags "by reading the text", or pattern-match notion URLs by hand — it's slow, (for hashing) impossible to do reliably, and (for URLs) brittle to Notion's four observed serialization variants. The helper emits the `sha256:` prefix so output drops straight into `local_hash` / `remote_hash`, and emits canonical dashed UUIDs from `extract-uuids` so equality compares work regardless of upstream form. `diff.md` and `commit.md` therefore carry `Bash(python3:*)` in `allowed-tools`.
12. **Page bodies never accumulate in the main command context.** Bulky read-and-hash work fans out to sub-agents that return **schema-validated** records — `CompactReadRecord` (status/pull/init/diff reads), `CommitWriteResult` (commit Modified writes), `DiffTextRecord` (diff render). All three schemas live in `references/sub-agent-schemas.md`. **For the ≥8-pages branch, dispatch via the `Workflow` tool's `agent(prompt, { schema, label, phase, agentType })` form** — the `schema` arg is what forces structured output. Plain `Agent`-tool dispatch is the v0 contract and was retired after a 24-agent run produced five different return shapes (see postmortem `/Users/stanley/.claude/plans/summarize-what-went-wrong-wobbly-sunrise.md`). Below the fan-out threshold (~8 pages) run inline with process-and-discard (hash via temp file under `.nsync/tmp/`, drop the body). All five commands carry `Task` AND `Workflow` in `allowed-tools`. The fan-out rules + schema mandate live ONLY in `references/notion-mcp-cheatsheet.md` → "Sub-agent fan-out & context discipline". Manifest persistence is per-batch/phase, not per-page.
13. **`notion-search` is NOT a tree-listing API.** It is semantic, relevance-ranked, and capped well below any safety limit regardless of pagination — empirically returns ~15 of 96 pages with `query="."` and `page_url=<parent>`. Never call `search` to enumerate "what pages exist under the parent." Derive reachability instead from the parent fetch's `<page url>` children unioned with every `has_children: true` PageRecord's freshly-fetched `child_link_tags`; that data is already in flight for the per-page hash step. `notion-search` retains a legitimate use for the init connectivity smoke-test. The trade-off (orphan-promoted pages disconnected from the tree don't appear) is acceptable because the existing `TrashEntry trashed_by` bookkeeping already covers them. See `references/notion-mcp-cheatsheet.md` → "Reads parallelize, writes serialize" → the `notion-search is NOT a tree-listing API` bullet.
...
| Hash / normalization / diff pipeline (the actual computation) | `references/manifest-schema.md` → "Markdown normalization" + "Compute helper" first, then `scripts/nsync.py` (keep them in lockstep), then re-run `python3 scripts/nsync.py self-test` |
| Context discipline / sub-agent fan-out / record schemas / Workflow dispatch | `references/notion-mcp-cheatsheet.md` → "Sub-agent fan-out & context discipline" + `references/sub-agent-schemas.md` first, then the fetch loops in `commands/{pull,status,diff,init,commit}.md` |
| URL → UUID extraction (any consumer of `<page url>` tags) | `scripts/nsync.py extract-uuids` is the canonical entry point; never write inline regex. The managed-line recognition regex in `references/path-mapping.md` → "Recognition regex" stays strict (it identifies *our* output, not Notion's) |
...
3. **Compute-helper self-tests.** `scripts/nsync.py` must reproduce the prose pipeline exactly. Run `python3 scripts/nsync.py self-test` — it covers `<empty-block/>` + unknown-tag stripping (invariant #6), child-link / `<page url>` invariance (invariant #2/#9), `extract-uuids` across all four observed URL forms (invariant #11), and image-line / NFC / CRLF normalization. After any regex/pipeline change, a Clean page must stay Clean (hash unchanged) and `self-test` must still exit `0`.
